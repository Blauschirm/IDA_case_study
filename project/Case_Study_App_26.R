  ...
  output$datatable_gemeinden <- renderDataTable({
    ...
    datatable(
      ...
      options = list(
        ...
        search = list(regex = FALSE, caseInsensitive = TRUE, search = "Koeln"), # ä=ae, ö=oe, ü=ue
  ...
  output$datatable_bauteile <- renderDataTable({
    ...
    datatable(
      ...
      options = list(
        ...
        # Search options
        search = list(regex = FALSE, caseInsensitive = TRUE, search = ""), # ä=ae, ö=oe, ü=ue
  ...
  output$map <- renderLeaflet({
    ...
    leaflet_map <- leaflet(final_joined) %>%
      ...
      addHeatmap(data = datapoints_heat(), lng = ~Längengrad, lat = ~Breitengrad,
                 intensity = ~fehleranzahl, blur = 12, max = 500, radius = 16, group = "Heatmap") %>% # intensity = ~fehleranzahl, blur = 14, max = 60, radius = 12) %>%
