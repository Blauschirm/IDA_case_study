...
Das Ziel dieser Case Study ist die Verdeutlichung und Aufklärung von und über Produktionsproblemen eines Unternehmens "111", das Ledersitze für Automobilhersteller aus eingekauften Einzelteilen zusammenbaut. Nach Hinweis durch den Automobilhersteller auf Kundenreklamationen sollen von allen beteiltigten Schritten der Kette vom Einzelteilhersteller bis zum Kunden Daten zu defekten Fahrzeugen zusammengetragen, aufbereitet und ausgewertet werden.
...
Dazu galt es zunächst, alle für die Analyse und Fehleridentifizierung relevanten Datzensätze zu importieren. Diese entsprechen der Komponente "K2LE1" mit den Einzelteilen "T11", "T14" und "T15" sowie "K2LE2" mit den Einzelteilen "T16", "T19" und "T20", den Fahrzeugen "Typ11" und "Typ12" des Herstellers "OEM1" sowie "Typ21" und "Typ22" des Herstellers "OEM2" und die entsprechenden Geodaten für Werke der "Tier2" (Einzelteile), "Tier1" (Komponenten) und "OEM". Für die Zuordnung zu Fahrzeughaltern und die spätere visuelle Darstellung der räumlichen Verteilung der Defekte waren die Datensätze "Zulassungen" mit den entsprechenden Geodaten "Gemeinden" relevant.

Um die Datensätze zu einem zusammenfassen, standen die relationalen Tabellen "Bestandteile_Komponente" (für Einzeilteile und Komponenten) und "Bestandteile_Fahrzeug" (für Komponenten und Fahrzeuge) zur Verfügung.
...
Zusätzl ich soll die Auswahl und Ausgabe eines betroffenen Fahrzeugs für den Fahrzeughalter von den restlichen Informationen, die dem Fahrzeughersteller zur Verfügung stehen, aus Datenschutzgründen isoliert werden.
...
Das UI für Fahrzeughalter ist umfangreicher und stellt sämtliche Daten zu allen Fahrzeugen, die von defekten Ledersitzen des Herstellers "111" betroffen sind, und deren Analyse dar. Es beinhaltet die folgenden Teilbereiche:
- den Graphen
- Auswahlfilter


Layout und Aufbau (Header, Tabs, Plot, Filter, Kartendarstellung, Daten)
Header, Logo und Schriftart

### Suche nach betroffenen Fahrzeugen für Fahrzeughalter

Eingabe der Fahrzeug ID über Suchfeld
Output-Darstellung:
- Fahrzeug betroffen
- betroffen Teile
- Werksnummer
...
stellt den zeitlichen Zulassungsverlauf der betroffenen Fahrzeuge dar.
Barplot zur Darstellung der Häufigkeitsverteilung über die Monate in Anzahl der betroffenen Fahrzeugen


kann zeitlich und nach Gemeinden gefiltert werden

### Auswahl der Zulassungsdaten

sliderInput()
start = minimum, end = maximum
Zulassungszeitspanne kann ausgewählt werden
filtert Barplot und Kartendarstellung

### Suchfilter für Gemeinden, Bauteile und den vollständigen Datensatz

dataTableInput()

Gemeinden- und Bauteilfilter sowie vollständiger Datensatz

Genereller Suchfilter mit Texteingabe ermöglicht Suche nach PLZ oder Ort bzw. ID der Bauteile usw.

Auswahl der Zeilen filtert die Daten, die im Output dargestellt werden

Markierung der Auswahl als Highlights

Mehrfachauswahl möglich

einfache Abwahl durch erneutes Anklicken

Gemeinden-Suche filter Barplot und Kartendarstellung und zeigt zusätzlich die Anzahl der zugelassenen betroffenen Fahrzeuge je Gemeinde an

Bauteil-Suche filtern nur die Kartendarstellung, da sich der Barplot zur besseren Übersicht ausschließlich auf die zugelassenen Fahrzeuge nach OEM und Gemeinden beziehen soll

Zur Nutzerfreundlichkeit filtert die DataTable, welche den vollständigen Datensatz enthält, ebenfalls die Kartendarstellung 
...
Open Street Map mit Darstellung der Deutschlandkarte

Heatmap für Schadensschwerpunkte

Cluster-Marker, die für jeweils ein betroffenes Fahrzeug repräsentieren und den genauen Zulassungsort auf der Karte markieren

Popups auf den Cluster-Markern stellen folgende Infos zu den Fahrzeugen dar:
- Fahrzeug-ID
- ...

Lieferwege als Luftlinie abhängig von der Auswahl der Fahrzeuge bzw. Bauteile aus den Datentabellen
- Lieferkette von defekten Bauteilen zum Komponentenhersteller und vom Komponentenhersteller zum Fahrzeughalter
- Gebäude-Icons stellen Einzeilteil- und Komponentenwerk dar, roter Geo-Icon stellt den Zielort, d.h. den Zulassungsort des Fahrzeugs dar
- Kreise um Werke repräsentieren durch ihre jeweilige Größe die Anzahl der fehlerhaften Einzelteile und Komponenten, die aus dem jeweiligen Werk geliefert wurden
- Popups beim Klicken auf den Werksstandort geben wesentliche Statistiken zu fehlerhaften Bauteilen aus
- Mehrfachauswahl möglich, farbliche Unterscheidung zwischen den Lieferkette (eine Farbe pro Kette), beim Überfahren des Lieferweges mit der Maus wird die Linie farblich hervorgehoben und die Fahrzeug-ID angezeigt.

Die Heatmap, Cluster-Marker sowie Lieferwege sind einzeln oder mehrfach aus- oder abwählbar.
