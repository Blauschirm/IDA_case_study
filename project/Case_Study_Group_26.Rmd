...
fahrzeug_bestandteile <- bind_rows(bestandteile_oem1_typ11, bestandteile_oem1_typ12) %>%
  ...
  mutate(id_richtig_fahrz_bestandteile = isTRUE(
    ...
    strsplit(ID_Sitze, split = "-", fixed = TRUE)[[1]][2] == 
      substring((strsplit(ID_Sitze, split = "-", fixed = TRUE)[[1]][3]), 1, nchar(strsplit(ID_Sitze, split = "-", fixed = TRUE)[[1]][2])) &&
    #check if Herstellernr and number in Werksnummer match in ID_Fahrzeuge
    strsplit(ID_Fahrzeug, split = "-", fixed = TRUE)[[1]][2] == 
      substring((strsplit(ID_Fahrzeug, split = "-", fixed = TRUE)[[1]][3]), 1, nchar(strsplit(ID_Fahrzeug, split = "-", fixed = TRUE)[[1]][2]))
  ))
