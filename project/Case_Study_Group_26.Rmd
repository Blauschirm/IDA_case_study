...
fahrzeuge <- bind_rows(fahrzeug_oem1_typ11, fahrzeug_oem1_typ12) %>%
  bind_rows(fahrzeug_oem2_typ21) %>%
  bind_rows(fahrzeug_oem2_typ22) %>%
  mutate(id_richtig_fahrzeuge =
           isTRUE(
              #check if ID-Herstellernr and Herstellernr match
              strsplit(ID_Fahrzeug, split = "-", fixed = TRUE)[[1]][2] == Herstellernummer && 
              #check if ID-Werksnr and Werksnr match
              strsplit(ID_Fahrzeug, split = "-", fixed = TRUE)[[1]][3] == Werksnummer &&
...
fahrzeug_bestandteile <- bind_rows(bestandteile_oem1_typ11, bestandteile_oem1_typ12) %>%
  ...
  mutate(id_richtig_fahrz_bestandteile = 
                  #check if Herstellernr and number in Werksnummer match in ID_Sitze
           isTRUE(strsplit(ID_Sitze, split = "-", fixed = TRUE)[[1]][2] == as.numeric(substring((strsplit(ID_Sitze, split = "-", fixed = TRUE)[[1]][3]), 1, nchar(strsplit(ID_Sitze, split = "-", fixed = TRUE)[[1]][2]))) &&
                  #check if Herstellernr and number in Werksnummer match in ID_Fahrzeuge
                  strsplit(ID_Fahrzeug, split = "-", fixed = TRUE)[[1]][2] == as.numeric(substring((strsplit(ID_Fahrzeug, split = "-", fixed = TRUE)[[1]][3]), 1, nchar(strsplit(ID_Fahrzeug, split = "-", fixed = TRUE)[[1]][2])))))

#if all IDs are correct, deselect redundant columns
if(all(fahrzeug_bestandteile$id_r == TRUE) && all(fahrzeug_bestandteile$id_richtig_fahrz_bestandteile == TRUE)){
  fahrzeug_bestandteile <- fahrzeug_bestandteile %>%
    select(-id_r, -id_richtig_fahrz_bestandteile)
}

#join Fahrzeuge and Bestandteile
#transform dates in correct format
fahrzeuge_end <- full_join(fahrzeuge, fahrzeug_bestandteile, by = "ID_Fahrzeug") %>%
