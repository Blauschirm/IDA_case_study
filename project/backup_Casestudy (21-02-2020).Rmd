...
# Import datasets, clean ID from NA and rename columns
...
# identify number of zulassungen pro gemeinde
zulassungen_pro_gemeinde <- zulassungen %>%
  group_by(Gemeinde) %>%
  summarise(Anzahl = length(Gemeinde)) %>%
  arrange(desc(Anzahl))
...
komponente_k2le2 <- komponente_k2le2 %>%
  # format origin in date format and create Produktionsdatum, remove unnecessary columns, add Komponente columns to distingiush dataset later when joined
...
## Clean and tidy Bestandteile_Komponente datasets
...
# Join fahrzeuge with zulassungen and gemeinden_clean (containing coordinates), left joins to keep all fahrzeuge data and only relevant coordinates
fahrzeuge_geo <- left_join(fahrzeuge, zulassungen, by = "ID_Fahrzeug") %>%
  left_join(., gemeinden_clean, by = "Gemeinde")


## Clean joins by irrelevant columns and join into final dataset
...
# join einzelteile_, komponenten_, and fahrzeuge_clean by ID_Sitze column, remove Fehlerhaft column from fahrzeuge
