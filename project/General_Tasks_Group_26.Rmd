...
Es werden die benötigten Bibliotheken importiert. Die *select* Funktion aus dem *dplyr*-Paket und aus dem *fitdistrplus*-Paket stehen im Konflikt. Um sicher zu gehen, dass select vom *dplyr*-Paket verwendet wird, wird dies nochmal definiert.
...
Dem Graphen zufolge passen die logarithmische Normalverteilung, die Gammaverteilung und die Weibullverteilung ganz gut zu den Daten. Nun werden die drei Verteilungen miteinander verglichen.
...
Anhand der Graphen kann man ablesen, dass die Weibullverteilung nicht passt, denn die Punkte des QQ-Plots ergeben keine Gerade. Die anderen beiden Verteilungen repräsentieren die Datetn aber ganz gut. Dies sieht man ganz gut am QQ-Plot, denn die Punkte beider Verteilungen ergeben eine Gerade. Beim Vergleich stellt sich heraus, dass die logarithmische Normalverteilung am besten passt, denn beim QQ-Plot sind die Punkte der logarithmische Normalverteilung näher an der Geraden.
...
Wir stellen den Graphen als Histogram mittels *ggplot* dar. Dabei wird ein Untertitel verwendet, der angibt in welchen Zeitraum sich die Daten befinden. Dafür muss das älteste Datum und das jüngste Datum extrahiert werden und umformatiert werden. Der Mittelwert wird mithilfe einer roten gestrichelten Linie angezeigt. Für das Histogramm wird noch die Häufigkeit des Logistikverzuges angezeigt, denn bei sehr kleinen Daten kann man das nicht auf dem Histogramm ablesen. Die x-Achse wird nur auf die relevante Spanne eingeschränkt, also dem Minimum und Maximum der Daten.
...
ggplot(Logistikverzug, aes(x = Verzug)) +
  ...
  geom_vline(aes(xintercept=mean(Verzug)), color="red", linetype="dashed", size=1) +
...
Angenommen die Applikation würde auf diese Datensätze zugreifen müssen. Wenn diese Datensätze dann nur lokal auf meinem persönlichen Computer gespeichert sind, dann sind diese Datensätze nicht immer zugreifbar für die veröffentlichte Applikation, denn mein Computer ist nicht 24h lang aktiv.
...
Um das Bauteil zu finden, werden erstmal alle Bestandteile der Fahrzeuge in einer Tabelle zusammengefügt, damit diese nach dem gesuchten Bauteil gefiltert werden kann. Nun muss noch herausgefunden werden, wo das Fahrzeug mit dem gesuchten Bauteil zugelassen wurde. Da ein Bauteil nur in einem Fahrzeug verbaut sein kann, reicht es wenn man die Zulassungen aller Fahrzeuge mit dem zuvor gefilterten Daten zusammenfügt. Der Ort, an dem dieses Fahrzeug zugelassen wurden, steht in der Spalte *Gemeinden*. Jedoch sind die Characterstrings alle in Großbuchstaben geschrieben. Dies muss in die richtige Rechtschreibung konvertiert werden.
